    ...
    , ui(new Ui::ApplyFriend)
{
    ...
    connect(ui->more_lb, &ClickedLab::clicked, this, &ApplyFriend::ShowMoreLabel);
    InitTipLbs();
    //链接输入标签回车事件
    ...
    connect(ui->tip_lb, &ClickedLab::clicked, this, &ApplyFriend::SlotAddFirendLabelByClickTip);
...
void ApplyFriend::InitTipLbs()
{
    ...
    for (int i = 0; i < _tip_data.size(); i++) {
        ...
        connect(lb, &ClickedLab::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
...
void ApplyFriend::AddTipLbs(ClickedLab *lb, QPoint cur_point, QPoint &next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
//点击标已有签添加或删除新联系人的标签
void ApplyFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickedLabState state)
{
