...
ChatView::ChatView(QWidget* parent):QWidget(parent),isAppended(false) {
    ...
    pMainLayout->setContentsMargins(0, 0, 0, 0);  // 设置布局的边距

    m_pScrollArea=new QScrollArea();
    m_pScrollArea->setObjectName("chat_area");
    pMainLayout->addWidget(m_pScrollArea);  //将滚动区加入到垂直布局
...
void ChatView::insertChatItem(QWidget *brfore, QWidget *item)
{
...
void ChatView::removeAllItem()
{
    ...
    for(int i=0;i<count-1;i++){
        QLayoutItem* item=layout->takeAt(0);//始终重第一个控件开始删除
        if(item){
