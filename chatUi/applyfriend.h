...
public:
    ...
    ~ApplyFriend();
    void InitTipLbs();
    void AddTipLbs(ClickedLab*, QPoint cur_point, QPoint &next_point, int text_width, int text_height);
...
private:
    ...
    void resetLabels();
    //已经创建好的标签
    QMap<QString, ClickedLab*> _add_labels;
...
public slots:
    ...
    //通过点击tip实现增加和减少好友便签
    void SlotChangeFriendLabelByTip(QString, ClickedLabState);
    //输入框文本变化显示不同提示
