...
LogicSystem::LogicSystem()
{
	...
	RegPost("/user_register", [](std::shared_ptr<HttpConnection> connection) {
		...
		bool b_get_varify = RedisMgr::GetInstance()->GetInstance()->Get(
			PREFIX + src_root["email"].asString(), varify_code);
		if (!b_get_varify) {
		...
		//查找数据判断是否存在
		int uid = MysqlMgr::GetInstance()->RegUser(name, email, pwd,sex,icon);
		if (uid == 0 || uid == -1) {
	...
	RegPost("/reset_pwd", [](std::shared_ptr<HttpConnection> connection) {
		...
		std::string  varify_code;
		bool b_get_varify = RedisMgr::GetInstance()->Get(PREFIX + src_root["email"].asString(), varify_code);
		if (!b_get_varify) {
			std::cout << " get varify code expired" << std::endl;
			root["error"] = ErrorCodes::VarifyExpired;
			std::string jsonstr = root.toStyledString();
	...
	RegPost("/user_login", [](std::shared_ptr<HttpConnection> connection) {
		...
		if (!parse_success) {
			std::cout << "failed to parse Json data!" << std::endl;
			root["err"] = ErrorCodes::Error_Json;
			std::string jsonstr = root.toStyledString();
			beast::ostream(connection->_response.body()) << jsonstr;
			return true;
		}
		...
		UserInfo userInfo;
		//查询数据库判断邮箱和密码是否匹配
		bool pwd_valied = MysqlMgr::GetInstance()->CheckPwd(email, pwd, userInfo);
		...
		std::cout << "succed to load userinfo uid is" << userInfo.uid << std::endl;
